...
class DataProcessor:
    ...
    def scatter_plot(self, type, bottom, top):
        ...
        if type in ['Minus_MIE', 'Minus_DI', 'Minus_MM']:
            ...
            ylabel = f"{type} (haven't summed up)"
        else:
...
for i in range(2, 16):
    NCREE = DataProcessor(f'七層鋼構架樓層破壞_濾波_9601-13600_{i}_3_3', 8, i)
    NCREE.calculate_DI()
    NCREE.minus("MIE") 
    NCREE.minus("DI")  
    NCREE.sum_up("MIE")
    NCREE.sum_up("DI")
    NCREE.sum_up("Minus_MIE")
    NCREE.sum_up("Minus_DI")
    NCREE.scatter_plot("MIE", -5, 5)
    NCREE.scatter_plot("DI", -3, 3)
    NCREE.scatter_plot("Minus_MIE", -3, 3)
    NCREE.scatter_plot("Minus_DI", -3, 3)
    NCREE.histogram("MIE", -0, 20)
    NCREE.histogram("DI", -2, 2)
    NCREE.histogram("Minus_MIE", -5, 5)
    NCREE.histogram("Minus_DI", -2, 2)
    NCREE.threeD("MIE", 2.5, 5)
    NCREE.threeD("DI", -2, 2)
    NCREE.threeD("Minus_MIE", -5, 5)
